...
This isn't a post about how to use the iOS-provided [text styles](https://developer.apple.com/documentation/swiftui/font/textstyle) or how to use a [custom font](https://www.hackingwithswift.com/quick-start/swiftui/how-to-use-dynamic-type-with-a-custom-font) and support text scaling. This blog assumes you already have a text system in your app that scales. Supporting dynamic type isn't just about your text getting bigger. To support it fully you need to scale other elements of your UI, allow the text room to grow, and think carefully about what should and should not get larger. Allowing all of your text to get as big as it needs is a naive approach that would likely make your app experience worse. But so too is limiting text where scaling doesn't work well. Instead, it's worth spending a few minutes considering where it makes sense to scale as much as needed, and where we should allow an alternative.


## 1. All text content must be inside a scroll view

There's only one way to be certain that your text content will have enough space to grow without checking every combination of physical screen size, screen zoom, text scale, custom font, or text weight. And that is to use scroll views. Don't make everything in your app a scroll views all the way down — one per screen is usually the perfect number — but for each text element, somewhere above it in the view hierarchy there must be a scroll view. With one exception:
...
Applying the `.lineLimit()` modifier to text that appears on a detail screen means your users will lose out on content. It will be truncated. On a detail screen users will expect to see the full detail. If you are cutting off some text entirely on these screens this is not an inconvenience, it makes your app entirely unusable.


## 6. Use .lineLimit() on list screens that push to detail

The proper use of the `.lineLimit()` modifier is on list or collection screens. Here the purpose of the content is to show just enough so your user has the context to know if they would like to tap this item to push to a detail screen with the full content. If you're providing all the content in a list, then your users are going to have to do a lot of scrolling to find the content they want, making navigation frustrating. So your larger text users will thank you for truncating text on these screens.
...
If you're using custom iconography, you can scale the size using the `@ScaledMetric` property wrapper. The code below draws a red rectangle that is 50 x 50 at the standard text size, but by applying `@ScaledMetric` this rectangle will grow and shrink relative to the `subheadline` text style. This technique can be applied to any UI that would benefit from scaling such as status indicators and increasing padding at larger text sizes.
...
Dynamic Type isn't the only thing that governs how much space your text occupies relative to screen size. In Settings > Appearance you'll find settings for Bold Text and Display Zoom. When you're testing, remember to enable these too. For Display Zoom, following good responsive design principles — in addition to what we've covered above — is the best option
